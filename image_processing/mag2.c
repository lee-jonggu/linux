	...
	for(i=0; i<height*elemSize; i+=elemSize) { 
		for(j=0; j<width*elemSize; j+=elemSize) {
            for(int z=0; z < elemSize; z++) {
                    ...
                    int e2 = inimg[j + (i * width)+elemSize+z];
                    int e3 = inimg[j + ((i+elemSize) * width)+z];
                    int e4 = inimg[j + ((i+elemSize) * width) + elemSize+z];

                    outimg[(j + (i * width * yFactor)) * xFactor + z] = e1;
                    outimg[(j + (i * width * yFactor)) * xFactor + elemSize + z] = (e1+e2)>>1;
                    outimg[(j + ((i * yFactor + elemSize )*width)) * xFactor + z] = (e1+e3)>>1;
                    outimg[(j + ((i * yFactor + elemSize) * width)) * xFactor + elemSize + z] = (e1+e2+e3+e4)>>2;
